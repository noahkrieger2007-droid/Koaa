...
/** Langsam rotierendes rundes Text-Badge im Hero. */
function RotatingBadge() {
  return (
    <div className="pointer-events-none absolute bottom-10 right-6 hidden h-32 w-32 md:block lg:right-16">
      <svg viewBox="0 0 100 100" className="animate-spin-slow h-full w-full" aria-hidden="true">
        <defs>
          <path id="badge-circle" d="M 50,50 m -38,0 a 38,38 0 1,1 76,0 a 38,38 0 1,1 -76,0" />
        </defs>
        <text className="fill-olive text-[10.5px] uppercase" style={{ letterSpacing: '0.22em' }}>
          <textPath href="#badge-circle">
            Kooa Concept · Saarbrücken · Est. Fröschengasse ·
          </textPath>
        </text>
        <circle cx="50" cy="50" r="4" className="fill-matcha" />
      </svg>
    </div>
  )
}
...
export default function Home() {
  ...
  // Parallax: Blobs & Inhalt driften beim Scrollen unterschiedlich schnell
  ...
  return (
    <PageWrapper>
      ...
      <section
        ref={heroRef}
        className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-beige px-5 text-center"
      >
        {/* organische, langsam morphende Hintergrund-Blobs mit Parallax */}
        <motion.div
          style={{ y: blobY }}
          className="animate-blob pointer-events-none absolute -left-32 top-24 h-96 w-96 rounded-blob bg-matcha/15"
        />
        <motion.div
          style={{ y: blobY2 }}
          className="animate-blob-slow pointer-events-none absolute -right-24 bottom-16 h-80 w-80 rounded-blob bg-olive/10"
        />
        <div className="animate-blob pointer-events-none absolute right-1/4 top-16 h-24 w-24 rounded-full bg-brown/5" />
        ...
        <motion.div style={{ y: contentY, opacity: contentOpacity }} className="relative flex flex-col items-center">
          <motion.div
            ...
            className="relative"
          >
            <span className="animate-float mx-auto flex h-24 w-24 items-center justify-center rounded-full bg-olive font-serif text-4xl font-bold text-cream shadow-lg shadow-olive/20 md:h-28 md:w-28 md:text-5xl">
              K
            </span>
          ...
          >
            ...
            <br />
            <Squiggle>
              <em className="text-olive">matcha</em>
            </Squiggle>{' '}
            spot.
          </motion.h1>
          ...
          >
            Slow mornings, guter Kaffee, kleine Rituale. Coffeeshop &amp;
            Concept Store in der Fröschengasse — same good vibes, jeden Tag.
          </motion.p>
          ...
          >
            <ArrowLink to="/menu" variant="solid">
              Zum Menü
            </ArrowLink>
            <ArrowLink to="/concept" variant="outline">
              Das Concept
            </ArrowLink>
          </motion.div>
        </motion.div>
        ...
        <div className="mx-auto grid max-w-6xl items-center gap-12 md:grid-cols-2 md:gap-20">
          <Reveal>
            ...
            <p className="mt-6 max-w-lg text-lg leading-relaxed text-brown/70">
              ...
              Hier gibt&apos;s Matcha-Rituale und Specialty Coffee,
              handgemachten Schmuck aus Workshops und Tattoos am Wochenende.
              Neue Getränke. Neues Deli. Immer dieselbe gute Energie.
            </p>
            <Link
              to="/concept"
              className="group/more mt-8 inline-block text-sm font-semibold text-olive underline decoration-matcha decoration-2 underline-offset-8 transition-colors hover:text-matcha"
            >
        ...
        <div className="mx-auto max-w-6xl">
          <Reveal className="text-center">
            ...
            <h2 className="mt-4 font-serif text-4xl font-bold text-brown md:text-5xl">
              Dein Morgen. <Squiggle><em className="text-olive">Dein Matcha.</em></Squiggle>
            </h2>
          </Reveal>
          ...
          <div className="mt-16 grid gap-8 md:grid-cols-3">
            {featured.map((item, i) => (
              <Reveal key={item.name} delay={i * 0.15}>
                <motion.article
                  whileHover={{ y: -8, rotate: item.rotate }}
                  transition={{ type: 'spring', stiffness: 250, damping: 20 }}
                  className="group flex h-full flex-col overflow-hidden rounded-3xl bg-cream shadow-sm transition-shadow duration-500 hover:shadow-xl hover:shadow-olive/10"
                >
                  <ImagePlaceholder
                    label={item.img}
                    tone={item.tone}
          ...
          <Reveal delay={0.2} className="mt-14 text-center">
            <ArrowLink to="/menu" variant="outline">
              Ganzes Menü ansehen
            </ArrowLink>
          </Reveal>
        </div>
      ...
      <section className="relative overflow-hidden bg-olive px-5 py-24 text-cream md:py-32">
        <div className="animate-blob pointer-events-none absolute -right-20 -top-20 h-72 w-72 rounded-blob bg-matcha/25" />
        <div className="animate-blob-slow pointer-events-none absolute -bottom-24 -left-16 h-80 w-80 rounded-blob bg-cream/10" />
        ...
        <Reveal className="relative mx-auto max-w-3xl text-center">
          ...
          <div className="mt-10">
            <ArrowLink to="/contact" variant="cream" className="font-semibold">
              So findest du uns
            </ArrowLink>
          </div>
        </Reveal>
